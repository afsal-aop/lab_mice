...
loader.load('assets/mouse.glb', function(glb) {
    ...
    if (glb.animations && glb.animations.length > 0) {
        ...
        // Identify animations by name
        let idleClip, injectClip;
        glb.animations.forEach(animation => {
            if (animation.name === "rig|rig|idol animtion") {
                idleClip = animation;  // Idle animation
            } else if (animation.name === "rig|jump") {
                injectClip = animation;  // Injection animation
            }
        });
        
        // Fallback to first animation if specific ones not found
        if (!idleClip) idleClip = glb.animations[0];
        if (!injectClip) injectClip = glb.animations[1] || glb.animations[0];
        ...
        injectAction = mixer.clipAction(injectClip);
        injectAction.loop = THREE.LoopOnce;
        injectAction.clampWhenFinished = true;
    }
}, undefined, function(error) {
    console.error('Error loading model:', error);
});
...
injectButton.addEventListener("click", () => {
    if (selectedComponent && selectedDose) {
        ...
        // Play injection animation if available
        if (injectAction && mixer) {
            // Disable button during injection
            injectButton.disabled = true;
            
            // Stop idle animation and play injection animation
            idleAction.stop();
            injectAction.reset();
            injectAction.play();
            
            // Create a one-time event listener for animation completion
            const onAnimationFinished = () => {
                mixer.removeEventListener('finished', onAnimationFinished);
                idleAction.play();  // Return to idle animation
                injectButton.disabled = false;  // Re-enable button
            };
